...
const DrawerList = ({ menu, menu2, toggleDrawer }: DrawerListProps) => {
  ...
  return (
    <div className="h-full">
      <div className="flex flex-col justify-between h-full w-[300px] border-r py-5 border-r-gray-200">
        {/* <div> */}
          <div className="space-y-2">
            {menu.map((item, index) => (
              <div onClick={() => navigate(item.path)} className="pr-9 cursor-pointer" key={index}>
                <p className={`${item.path == location.pathname ? "bg-teal-600 text-white" : "text-teal-600"}
